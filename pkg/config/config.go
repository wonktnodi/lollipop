...
import (
    ...
    "github.com/wonktnodi/lollipop/pkg/encoding"
    "fmt"
    "strings"
)
...
// Backend defines how service should connect to the backend service (the API resource to consume)
...
type Endpoint struct {
    ...
    Backend         [] BackendPoint `json:"backend"`
}
...
type Settings struct {
    ...
    DefaultMaxRate   int        `json:"default_max_rate"`
    ClientMaxRate    int        `json:"client_max_rate"`
    ThrottlingHeader string     `json:"throttling_header"`
...
func InitConfig() {
    ...
    if err := Cfg.Init(); err != nil {
        log.Fatalf("Error init system config, %s", err)
    }
}
...
func (e *EndpointConfig) validate() error {
    ...
    if err != nil {
        log.Errorf("ERROR: parsing the endpoint url [%s]: %s. Ignoring\n", e.Endpoint, err.Error())
        return err
    }
    if matched {
