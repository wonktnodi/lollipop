...
func TestRender_Negotiated_ok(t *testing.T) {
	...
	for _, testData := range [][]string{
		...
		if w.Result().Header.Get("X-Lollipop") != "Version undefined" {
			t.Error(testData[0], "X-Lollipop error:", w.Result().Header.Get("X-Lollipop"))
		}
...
func TestRender_string_noData(t *testing.T) {
	...
	for k, p := range []proxy.Proxy{
		...
		if w.Result().Header.Get("X-Lollipop") != "Version undefined" {
			t.Error(k, "X-Lollipop error:", w.Result().Header.Get("X-Lollipop"))
		}
		if w.Result().StatusCode != http.StatusOK {
...
func TestRender_noop(t *testing.T) {
	...
	if w.Result().Header.Get("X-Lollipop") != "Version undefined" {
		t.Error("X-Lollipop error:", w.Result().Header.Get("X-Lollipop"))
	}
	if w.Result().StatusCode != http.StatusOK {
